...
export const streamSessions = pgTable(
  ...
  (table) => [
    // Crash recovery queries sessions by status on every service start.
...
export const recordings = pgTable(
  ...
  (table) => [index('recordings_session_id_idx').on(table.sessionId)],
);

export const streamDestinations = pgTable(
  'stream_destinations',
  {
  ...
  (table) => [
    // Postgres does not auto-index FK columns; every findBySessionId /
    // findByIngestKey does a second lookup on session_id.
    index('stream_destinations_session_id_idx').on(table.sessionId),
  ],
);
